...
public class ScreenComponentUpgradeInfo extends ScreenComponent {
	...
	public ScreenComponentUpgradeInfo(IScreenWrapper gui, int x, int y, int[] screenNumbers, Supplier<IUpgradableTile> tile) {
	...
	public void renderBackground(PoseStack stack, int xAxis, int yAxis, int guiWidth, int guiHeight) {
		...
		if(owner != null) {
			...
			if(currSpeed > 0 && owner.getDefaultSpeed() > 0) {
				component = new TranslatableComponent("gui.matteroverdrive.time", UtilsFormatting.formatTimeValue(operatingTime / currSpeed / 20.0));
			...
			if(currPowerUsage > 0 && owner.getDefaultPowerUsage() > 0) {
				component = new TranslatableComponent("gui.matteroverdrive.usage", UtilsFormatting.formatPowerValue(currPowerUsage));
			...
			if(currMatterUsage > 0 && owner.getDefaultMatterUsage() > 0) {
				component = new TranslatableComponent("gui.matteroverdrive.usage", UtilsFormatting.formatMatterValue(currMatterUsage));
			...
			if(owner.getDefaultFailure() > 0) {
				component = new TranslatableComponent("gui.matteroverdrive.failure", UtilsFormatting.formatPercentage(failureChance * 100));
			...
			if(currPowerStorage > 0 && owner.getDefaultPowerStorage() > 0) {
				component = new TranslatableComponent("gui.matteroverdrive.storage", UtilsFormatting.formatPowerValue(currPowerStorage));
			...
			if(currMatterStorage > 0 && owner.getDefaultMatterStorage() > 0) {
				component = new TranslatableComponent("gui.matteroverdrive.storage", UtilsFormatting.formatMatterValue(currMatterStorage));
				color = currMatterStorage >= owner.getDefaultMatterStorage() ? GREEN : RED;
				font.draw(stack, component, guiWidth + this.xLocation, guiHeight + this.yLocation + offset, color);
				offset += 10;
			}
			if(owner.isMuffled(true)) {
				font.draw(stack, new TranslatableComponent("gui.matteroverdrive.soundmuted"), guiWidth + this.xLocation, guiHeight + this.yLocation + offset, GREEN);
			}
