...
public class TileTritaniumCrate extends GenericTile {
	...
	public TileTritaniumCrate(BlockPos pos, BlockState state) {
		...
		addCapability(new CapabilityInventory(SIZE).setOwner(this));
		
		setMenuProvider(new SimpleMenuProvider((id, inv, play) -> {
			return new InventoryTritaniumCrate(id, play.getInventory(), exposeCapability(CapabilityType.Item), getCoordsData());
	...
	public static enum CrateColors implements IBulkRegistryObject {
		...
		DARK_GREY, GREEN, LIGHT_BLUE, LIGHT_GREY, LIME_GREEN, MAGENTA, 
		ORANGE, PINK, PURPLE, RED, REG, WHITE, YELLOW;
	}
