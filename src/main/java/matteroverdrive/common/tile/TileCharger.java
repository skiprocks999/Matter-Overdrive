...
public class TileCharger extends GenericRedstoneSoundTile<TileCharger> {
	...
	public TileCharger(BlockPos pos, BlockState state) {
		...
		this.addInventory(
				input = (SidedInventoryComponent<TileCharger>) new SidedInventoryComponent<TileCharger>("input", 10, 20,
						1, 0).setInputFilter((stack, slot) -> {
							...
							return false;
						}).setOutputFilter((stack, slot) -> false).setComponentHarness(this));
		this.addInventory(
				output = (SidedInventoryComponent<TileCharger>) new SidedInventoryComponent<TileCharger>("output", 30,
						20, 1, 0).setInputFilter((stack, slot) -> false).setOutputFilter((stack, slot) -> false)
	...
	public void serverTick(Level level, BlockPos pos, BlockState state, TileCharger blockEntity) {
		...
		if (stack.getCapability(CapabilityEnergy.ENERGY).isPresent()) {
			...
			if (lazyStorage.isPresent() && lazyStorage.resolve().isPresent()) {
				...
				boolean canReceive = energyStorage.canReceive() && energyStorageComponent.canExtract()
